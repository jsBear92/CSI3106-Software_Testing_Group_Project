...
public class Planner {
	...
	public void mainMenu() {
		...
		System.out.println("5. Check agenda for a room");
	...
	public void checkRoomAvailability() {
		...
		System.out.println("The rooms available at the specified time are:");
		//Display the rooms open at that time
		for(Room r: org.getRooms()){
			try{
				if(!(r.isBusy(month, day, startTime, endTime))){
	...
	public void checkAgendaRoom() {
		...
		if(!name.equals("cancel")){
			try{
				Room room_no = org.getRoom(name);
				// System.out.println("Room ID is "+room_no.getID());
				if(!day.equals("all")){
					System.out.println(room_no.printAgenda(month, Integer.parseInt(day)));
				}else{
					System.out.println(room_no.printAgenda(month));
				}
			}catch(Exception e){
				System.out.println(e.getMessage());
			}
	...
	public void checkAgendaPerson() {
		...
		//Display the names of people available at that time
		for(Person p: org.getEmployees()){
			System.out.println(p.getName());
		}
